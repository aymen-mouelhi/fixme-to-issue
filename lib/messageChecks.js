...
try {
  ...
  if(options.annotations){
    options.annotations = options.annotations.map(function(annotation){
      annotation.regex =  new RegExp('/[\/\/][\/\*]\s*'+ annotation.name + '\s*(?:\(([^:]*)\))*\s*:?\s*(.*)/i')
      annotation.colorer = chalk[annotation.color];
