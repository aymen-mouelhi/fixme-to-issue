    ...
    constructor() {
        ...
        if (this.repository) {
          ...
          this.getIssues().then(function(issues){
            self.issues = issues;
            console.log(chalk.blue(`Issues: ${JSON.stringify(self.issues)}`));
          });

          // Get Contributors
          this.getContributors().then(function(contributors){
            self.contributors = contributors;
            console.log(chalk.blue(`Contributors: ${JSON.stringify(self.contributors)}`));
          })
        }

        console.log(chalk.blue(`Url: ${this.url} \nAuthor: ${this.author} \nRepository: ${this.repository}`));
    }
