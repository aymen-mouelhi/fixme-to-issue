...
try {
    pkg = JSON.parse(require('fs').readFileSync(require('path').resolve(process.cwd(), './package.json'), 'utf8'));
    if (!pkg.repository) {
      pkg = null;
    }
} catch (error) {
...
class Github {
    constructor(options) {
      if (!pkg) {
        console.log(chalk.red('You should add a package.json file that contains the url of your github repository'));
        process.exit();
      }
                    ...
                    if (found.githubLabel) {
                        if (typeof found.githubLabel === "object") {
                            for (var i = 0; i < found.githubLabel.length; i++) {
                              if (_.indexOf(labels, found.githubLabel[i]) < 0) {
                                labels.push(found.githubLabel[i]);
                              }
                            }
                        } else {
                          if (_.indexOf(labels, found.githubLabel) < 0) {
