...
class Github {
    constructor(options) {
        // TODO: Emit event when github ready [Issue #undefined]
        ...
        if (this.repository) {
            // TODO: Async waterfall and then emit event [Issue #undefined]
            // Get Issues
            this.getIssues().then(function(issues) {
                self.issues = issues;
                console.log(chalk.yellow(`The repository has ${issues.length} issue(s)`));
                // Emit Event
            ...
            this.getContributors().then(function(contributors) {
                ...
                console.log(chalk.yellow(`The repository has ${contributors.length} contributor(s)`));
            });
    ...
    createIssue(message) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          if(issue){
            console.log(chalk.yellow(`Issue ${issue.number} already exists in Github`))
            reject(`Issue ${issue.number} already exists in Github`);
          }
          ...
          axios.post(`${GITHUB_API_URL}/${self.author}/${self.repository}/issues`, {
              ...
              .then(function(response) {
                  //TODO: Get more information about the created issue [Issue #undefined]
