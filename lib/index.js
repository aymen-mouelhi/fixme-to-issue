        ...
        .pipe( eventStream.map( function( fileLineString, cb ){
          ...
          if (fileLineString.length < lineLengthLimit) {
            ...
            messages.forEach(function (message) {
              // TODO: Create Issue if doesn't exist
                ...
                if (github.isRepository()) {
                  if (message.message.indexOf(' [Issue #') < 0) {
                    ...
                    github.createIssue(message).then(function(issue){
                      console.log('Issue posted to github !');
                      try {
                      ...
                      console.log('[Index] Error while creating issue ! ' + error);
                    });

                  }else{
                    console.log('message contains  [Issue #')
                  }

                  fileMessages.messages.push(message);
                }else{
                  console.log('Current project is not a github project !')
                  fileMessages.messages.push(message);
                }
              }else{
                console.log('No .fixme-to-issue was found')
                fileMessages.messages.push(message);
              }
            });
  ...
  if (options) {
    ...
    if (options.github) {
      ...
      github.getEventEmitter().on('githubReady', function(){
        console.log(chalk.green('Github class has been initialized !'));
...
// TODO: options: github.username, github.password => Get from Git Config
// TODO: Create a github issue
// TODO: Check if issue has been already created
// TODO: add option: Labels => github lables
// TODO: how to deal with assigning issues? TODO @me => issue assigned to me;
// TODO @aymen => get list of contributors and match aymen with one of the contributors
// TODO: add option: if issue is closed => remove annotated line
// TODO: Add Gitlab Support
// TODO: Add Trello Support
