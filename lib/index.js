...
var ignoredDirectories  = ['node_modules/**', '.git/**', '.hg/**', 'bower_components/**'],
              ...
              if (github) {
                if (github.isRepository()) {
                  if (message.message.indexOf(' [Issue #') < 0) {
                    var remoteFilePath = github.getLink() +'/blob/' + github.getBranch() + '/' + fileMessages.path + '#L' + message.line_number;
                    message.description = `I have found a ${message.label.toLowerCase()} in the file [${fileMessages.path}](${remoteFilePath}) at line ${message.line_number}: ${message.message}`;
                    // Create Issue
