...
var byline        = require('byline'),
    ...
    messageChecks = require('./messageChecks');

var ignoredDirectories  = ['node_modules/**', '.git/**', '.hg/**'],
    filesToScan         = ['**/*.js', 'Makefile', '**/*.sh'],
    scanPath            = process.cwd(),
    fileEncoding        = 'utf8',
    github              = null, // Will be initialized later
 ...
 * @return  {Boolean}
 */
// TODO: This could be simpler using minimatch negation patterns in one set, instead disparate ones for files and directories. [Issue #undefined]
...
function scanAndProcessMessages () {
  ...
  // TODO: Actually do something meaningful/useful with these handlers. [Issue #99]
  stream
        ...
        .pipe( eventStream.map( function( fileLineString, cb ){
          ...
          if (fileLineString.length < lineLengthLimit) {
            ...
            messages.forEach(function (message) {
              ...
              console.log(`Path ${fileMessages.path}`);
              */
              if (github) {
                if (github.isRepository()) {
                  if (message.message.indexOf(' [Issue #') < 0) {
                    // Update description
                    // TODO: find a better way to enhance description
                    var remoteFilePath = github.getRepository() +'/blob/' + github.getBranch() + '/' + fileMessages.path;
                    //message.description = 'I have found a ' + message.label.toLowerCase() + ' in the file ['+ fileMessages.path + '](' + remoteFilePath + ') at line ' + message.line;
                    message.description = `I have foudn a ${message.label.toLowerCase()} in the file [${fileMessages.path}](${remoteFilePath}) at line ${message.line}:\n ${message.message}`;
                    // Create Issue
                    github.createIssue(message).then(function(issue){
                      try {
                        let changedFiles = replace.sync({
                          files: fileMessages.path,
                          from: message.message,
                          to: message.message + ' [Issue #' + issue.number +']'
                        });
                      } catch (error) {
                        console.log('[Index] Error occurred when trying to replace message:' +  error);
                      }
                    });
                  }
                }
...
function parseUserOptionsAndScan (options) {
  if (options) {
    ...
    if(messageChecks){
      options.annotations = messageChecks;
    }
    ...
    if (options.github) {
      ...
      github.getEventEmitter().on('githubReady', function(){
        console.log(chalk.green('[Index] Github class has been initialized !'));
        scanAndProcessMessages();
      });

      // Emit Event
      github.getEventEmitter().on('githubError', function(){
        //console.log(chalk.red('Github error occured !'));
        github = undefined;
        scanAndProcessMessages();
      });
    }else{
      console.log(chalk.red('No github options were passed to index !'))
      scanAndProcessMessages();
    }
  }
}
...
// TODO: how to deal with assigning issues? TODO @me => issue assigned to me; [Issue #98]
// TODO @aymen => get list of contributors and match aymen with one of the contributors [Issue #89]
// TODO: add option: if issue is closed => remove annotated line [Issue #93]
// TODO: Support Grunt tasks [Issue #102]
