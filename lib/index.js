            ...
            messages.forEach(function (message) {
              ...
              if (github) {
                if (github.isRepository()) {
                  ...
                  // Create Issue
                  github.createIssue(message).then(function(){
                    console.log('Issue posted to github !');
                    try {
                      let changedFiles = replace.sync({
                        files: fileMessages.path,
                        from: message.message.substr(0, message.message.length -10),
                        to: message.message.substr(message.message.length -10, message.message.length)
                      });
                    } catch (error) {
                      console.error('Error occurred:', error);
                    }
                  }).catch(function(error){
                    console.log('[GitHub] Error while creating issue !');
                  });
